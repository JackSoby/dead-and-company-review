...
def self.api_2016_fetch
  ...
  response2 = HTTParty.get('http://api.setlist.fm/rest/0.1/artist/94f8947c-2d9c-4519-bcf9-6d11a24ad006/setlists.json?p=3',
  :headers => { "accept" => "application/json", "x-api-key" => "#{ENV["SETLIST_API_KEY"]}"})
  response3 = HTTParty.get('http://api.setlist.fm/rest/0.1/artist/94f8947c-2d9c-4519-bcf9-6d11a24ad006/setlists.json?p=5',
  :headers => { "accept" => "application/json", "x-api-key" => "#{ENV["SETLIST_API_KEY"]}"})
  ...
  @dead1 = JSON.parse(response1.body)
  @dead2 = JSON.parse(response2.body)
  @dead3 = JSON.parse(response1.body)
  @dead4 = JSON.parse(response2.body)

    @shows = Array.new()
    @dead4["setlists"]['setlist'].each do |show|
        if show['@eventDate'].include?('2016') == true
      @shows.push("#{show["venue"]['@name']} " + "#{show['@eventDate']}")
    else
  end
end
    @dead3["setlists"]['setlist'].each do |show|
        if show['@eventDate'].include?('2016') == true
      @shows.push("#{show["venue"]['@name']} " + "#{show['@eventDate']}")
    else
  end
